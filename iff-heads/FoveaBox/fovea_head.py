...
class FoveaHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        if not self.with_deform:
            ...
            self.back = nn.Conv2d(self.cls_out_channels+4, chn, 3, padding=1)
        else:
            self.cls_convs.append(
                ConvModule(
            ...
            #new-back_layer
            self.back = nn.Conv2d(self.cls_out_channels+4,self.feat_channels, 3, padding=1)
    ...
    def init_weights(self):
        ...
        normal_init(self.back, std=0.01)
        if self.with_deform:
    ...
    def forward_single(self, x):
        ...
        #IFF
        x = x + self.back(self.leakyrelu(torch.cat((cls_score, bbox_pred), dim=1)))
