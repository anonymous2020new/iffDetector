...
class RetinaNetHead(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #new-back_layer
        self.back = nn.Conv2d(num_anchors * (num_classes + 4), 256, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        bbox_reg = []
        for feature in x:
            #IFF: the effects of ReLU and Sigmoid are about the same.
            feature = feature + self.back(torch.sigmoid(torch.cat((self.cls_logits(self.cls_tower(feature)), self.bbox_pred(self.bbox_tower(feature))), 1)))
